...
describe('runner', () => {
    ...
    describe('run', () => {
        ...
        describe('handling of events', () => {
            ...
            const testCoverage = (event) => {
                ...
                it('should be emitted after saving of coverage', () => {
                ...
                it('should not collect coverage if it is disabled', () => {
                    stubBrowserRunner((runner) => runner.emit(event));
            ...
            describe('on', () => {
                ...
                describe('END_SUITE', () => {
                    it('should be monitored by suite monitor', () => {
                        sandbox.stub(SuiteMonitor.prototype, 'suiteFinished');

                        stubBrowserRunner((runner) => runner.emit(Events.END_SUITE, {suite: 'foo', browserId: 'bar'}));

                        return run(createRunner())
                            .then(() => {
                                assert.calledOnce(SuiteMonitor.prototype.suiteFinished);
                                assert.calledWith(SuiteMonitor.prototype.suiteFinished, 'foo', 'bar');
                            });
                    });

                    it('should be emitted by suite monitor', () => {
                        sandbox.stub(SuiteMonitor.prototype, 'suiteFinished', function() {
                            this.emit(Events.END_SUITE, {foo: 'bar'});
                        });

                        stubBrowserRunner((runner) => runner.emit(Events.END_SUITE, {}));

                        const runner = createRunner();
                        const onEndSuite = sinon.spy().named('onEndSuite');

                        runner.on(Events.END_SUITE, onEndSuite);

                        return run(runner)
                            .then(() => {
                                assert.calledOnce(onEndSuite);
                                assert.calledWith(onEndSuite, {foo: 'bar'});
                            });
                    });
                });
                ...
                [
                    ...
                    describe(`${event}`, () => {
                        testPassthrough(event, 'should be emitted');
                ...
                describe('TEST_RESULT', () => {
                    testPassthrough(Events.TEST_RESULT, 'should be emitted');

                    it('should passthrough "END_TEST" event', () => {
                    ...
                    it('should be emitted after "END_TEST" event', () => {
    ...
    describe('setTestBrowsers', () => {
        it('should redefine browsers in which to run tests', () => {
            const runner = createRunner();
        ...
        it('should not redefine browsers in which to run tests if they are not from config', () => {
            const runner = createRunner();
    ...
    describe('cancel', () => {
        it('should not create browser runners if cancellation was called before starting if a runner', () => {
            const runner = createRunner();
