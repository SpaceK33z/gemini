...
const signalHandler = require('./signal-handler').create();
...
module.exports = class Gemini extends PassthroughEmitter {
    ...
    _run(stateProcessor, paths, options) {
        ...
        return getTests(paths, options)
            .then((suiteCollection) => {
                ...
                signalHandler.start();
                signalHandler.on(SignalEvents.EXIT, () => runner.cancel());

                return runner.run(suiteCollection)
                    .then(() => stats.get())
                    .finally(() => signalHandler.end());
            });
    }
    ...
    cancel(exitCode, msg) {
        if (!this._runner) {
            throw new GeminiError('Gemini was not started, nothing to cancel');
        }
