...
module.exports = class TestsRunner extends Runner {
    ...
    _formatBeginEventData(suiteCollection) {
        return {
            config: this.config,
            totalStates: _.reduce(suiteCollection.allSuites(), (total, suite) => total + suite.states.length, 0),
    ...
    _runTests(suiteCollection) {
        ...
        return _(this._getBrowsersToRun())
            .map((browserId) => !this._canceled && this._runTestsInBrowser(suiteCollection, browserId))
